...
void *
binocle_array_grow_internal(void *arr, uint64_t to_at_least, uint64_t item_size, const char *file, uint32_t line) {
  ...
  return binocle_array_set_capacity_internal(arr, new_capacity, item_size, file, line);
}

void *binocle_array_create_internal(const void *ptr, uint64_t size, uint64_t item_size,
                                    const char *file, uint32_t line) {
  void *arr = binocle_array_set_capacity_internal(0, size, item_size, file, line);
  binocle_array_header(arr)->size = size;
  if (ptr)
